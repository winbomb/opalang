...
(* detect function declaration tagged with @publish_env *)
let detect_candidate_def code = IdentSet.empty,code (* TODO *)
  ...
  let _, set = QmlAstWalk.CodeExpr.fold
    (QmlAstWalk.Expr.fold
       (fun (local,need_instrumentation) e ->
         ...
         (* ident cases *)
         | Q.Directive (a, `public_env , [Q.Ident(_, id)], _) ->
           (if IdentSet.mem id local then warn_unknown else warn_empty) a;
           force_rewrite:=true;
